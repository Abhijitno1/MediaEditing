...
namespace MediaEditingSample
{
    public sealed partial class Scenario1_TrimAndSaveClip : Page
    {
        ...
        private async void TrimClip_Click(object sender, RoutedEventArgs e)
        {
            ...
            var fromTimeAry = txtFromTime.Text.Trim().Split(':');
            int hrs, min, sec;
            if (fromTimeAry.Length == 3)
            {
              hrs = Convert.ToInt32(fromTimeAry[0]);
              min = Convert.ToInt32(fromTimeAry[1]);
              sec = Convert.ToInt32(fromTimeAry[2]);
            }
            else if (fromTimeAry.Length == 2) 
            {
              hrs = 0;
              min = Convert.ToInt32(fromTimeAry[0]);
              sec = Convert.ToInt32(fromTimeAry[1]);
            }
            else
            {
              hrs = 0;
              min = 0;
              sec = Convert.ToInt32(fromTimeAry[0]);
            }
            clip.TrimTimeFromStart = new TimeSpan(hrs, min, sec);

            var toTimeAry = txtToTime.Text.Split(':');
            if (toTimeAry.Length == 3)
            {
              hrs = Convert.ToInt32(toTimeAry[0]);
              min = Convert.ToInt32(toTimeAry[1]);
              sec = Convert.ToInt32(toTimeAry[2]);
            }
            else if (toTimeAry.Length == 2)
            {
              hrs = 0;
              min = Convert.ToInt32(toTimeAry[0]);
              sec = Convert.ToInt32(toTimeAry[1]);
            }
            else
            {
              hrs = 0;
              min = 0;
              sec = Convert.ToInt32(toTimeAry[0]);
            }
            clip.TrimTimeFromEnd = clip.OriginalDuration - new TimeSpan(hrs, min, sec);
